...
out="deploy/charts/milestone-operator/templates/crds.yaml"

# Order is significant only for a stable diff; keep it matching the chart.
plurals=(milestones clustermilestones)
...

{
  ...
  for plural in "${plurals[@]}"; do
    src="${bases}/milestone.as-code.io_${plural}.yaml"
    if [[ ! -f "$src" ]]; then
...
metadata:
  name: ${plural}.milestone.as-code.io
